...
class TreeBuilder {
    ...
    build() {
        ...
        this.buildBrackets(this.rootNode.children[0]);
    ...
    newNode(parent, token: Token, type: EXPRESSION_TYPE =null) {
        return Expression.NewFromToken(token, parent, type);
    }
    /* check for brackets
        * before: 
        * 
        ...
        *   operand1
            ...
            * after: op
                ...
                * operand2
    */
    buildBrackets(start: Expression, fromBracket = false) {
        console.log('building bracket on ' + start.value);
...
export class Expression extends TreeNode {
    ...
    static NewFromToken(token, parent, type= null):Expression {
        if (!type)
            type = Expression.calcTypeFromToken(token);
        return new Expression(type, parent, token.entry, token.index);
    }
    getState() {
        const children=[];
    ...
    displayExpression(level = 0) {
        ...
        this.loop(function (expr, level) {
            const msg = level + '-'.repeat(level + 1) + ">" + expr.type + " " + expr.value + "   " + (expr.result ? ':' + expr.result : '' + 'id:'+expr.id);
    ...
    buildTree(tokens,parent=null) {
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            if (token.group == 'literal' || (!isNaN(token.entry))) {
                ...
                childExpr = new Expression(type,this,token.entry,token.index);
    ...
    buildTreeOperators(tokens) {
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            if (token.isOperator()) {
                 ...
                 *
                 */
                    newExpr = new Expression(EXPRESSION_TYPE.Operator, this, token.entry,token.index);
    ...
    */
    ...
    buildTreeBirnary(operator,tokens) {
        ...
        for (i = 0; i < tokens.length; i++) {
            ...
            if (token.entry == operator) {
                ...
                const childExpr = new Expression(EXPRESSION_TYPE.Binary,this,token.entry,token.index);
    ...
    createChildFromTokens(type, value, tokens, parent) {
        ...
        if (tokens.length > 1) {
            childExpr = new Expression(type,parent,value,tokens[0].index);
...
export class Parser {
    ...
    compile(string) {
        ...
        /*
        const expr = new Expression(EXPRESSION_TYPE.Root, null, `<${string}>`,0);
